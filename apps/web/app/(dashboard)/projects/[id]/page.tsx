...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="bg-[#0b1215]/80 border border-white/10 backdrop-blur-xl rounded-2xl p-6 shadow-[0_14px_42px_rgba(0,0,0,0.16)]">
        <h2 className="text-xl font-bold mb-4 flex items-center space-x-2 text-white">
          <Clock className="w-5 h-5 text-[#00c9e8]" />
        ...
        {project.scans.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-[#cfe0ea]">No scans have been executed for this project yet.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {project.scans.map(scan => (
              <Link key={scan.id} href={`/scan/${scan.id}`} className="block">
                <div className="flex items-center justify-between p-4 bg-white/5 border border-white/10 hover:border-[#00c9e8]/40 hover:bg-white/10 rounded-xl transition-all duration-300 group">
                  <div className="flex items-center space-x-4">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center border shadow-sm ${
                      ...
                      "bg-[#00c9e8]/10 border-[#00c9e8]/20 text-[#00c9e8] shadow-[#00c9e8]/10"
                    }`}>
                    ...
                    <div>
                      <h4 className="font-bold text-white group-hover:text-[#00c9e8] transition-colors">
                        Scan {scan.id.split("-")[0]}...
                      </h4>
                      <p className="text-xs text-[#cfe0ea]/70">{new Date(scan.createdAt).toLocaleString()}</p>
                    </div>
                  </div>

                  <div className="flex items-center space-x-6">
                    <div className="text-right">
                      <p className="text-xs text-[#cfe0ea]/70 mb-0.5">Risk Level</p>
                      <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-bold border ${
                        ...
                        scan.riskLevel === "Low" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-white/5 border-white/10 text-slate-300"
                      }`}>{scan.riskLevel || "Unknown"}</span>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-[#cfe0ea]/70 mb-0.5">Status</p>
                      <p className="font-bold text-white capitalize">{scan.status}</p>
                    </div>
                  </div>
                </div>
              </Link>
